...
function UGFunctions(){
	...
	this.addFullScreenChangeEvent = function(func){
		this._fullScreenFunc = func;

		if(document["webkitCancelFullScreen"])
			addEvent("webkitfullscreenchange",document,this._fullScreenFunc);
		else if(document["msExitFullscreen"])
			addEvent("MSFullscreenChange",document,this._fullScreenFunc);
		else if(document["mozCancelFullScreen"])
			addEvent("mozfullscreenchange",document,this._fullScreenFunc);
		else
			addEvent("fullscreenchange",document,this._fullScreenFunc);
	}
	...
	this.destroyFullScreenChangeEvent = function(){
		...
		jQuery(document).unbind("fullscreenChange", this._fullScreenFunc);
		jQuery(document).unbind("mozfullscreenchange", this._fullScreenFunc);
		jQuery(document).unbind("webkitfullscreenchange", this._fullScreenFunc);
		jQuery(document).unbind("MSFullscreenChange", this._fullScreenFunc);
	}
...
function UGTouchThumbsControl(){
	...
	this.destroy = function(){
		...
		jQuery(window).add("body").unbind("mouseup touchend", onTouchEnd);
		jQuery("body").unbind("mousemove touchmove", onTouchMove);
	}
}
...
function UGAviaControl(){
	...
	function _mouseMove_s1(event){
		...
		if(g_temp.touchEnabled == true){
			jQuery("body").off("mousemove", _mouseMove_s1);
			return(true);
		}
	...
	function initEvents(event){
		...
		jQuery("body").on("touchstart", _touchStart_s1);
		
		//on body move
		jQuery("body").on('mousemove', _mouseMove_s1);
	...
	this.destroy = function(){
		...
		jQuery("body").off("touchstart", _touchStart_s1);
		jQuery("body").off("mousemove", _mouseMove_s1);
...
function UniteGalleryMain(){
	...
	function initEvents(){
		...
		if(g_options.gallery_mousewheel_role == "advance" && g_temp.isFreestyleMode == false)
		 ...
		 jQuery(window).resize(_winResize);
	...
	this.destroy = function(){
		...
		if(g_objThumbs){
		 ...
		 jQuery(window).unbind("resize", _winResize);
		 ...
		 //add keyboard events
		 if(g_options.gallery_control_keyboard == true)
			 jQuery(document).off("keydown", onKeyPress);
...
function UGLightbox(){
	...
	function initEvents(){
		...
		//on resize
		 jQuery(window).resize(_winResize1);
	...
	this.destroy = function(){
		...
		jQuery(window).unbind("resize", _winResize1);
		g_objGallery.off(g_gallery.events.GALLERY_KEYPRESS, onKeyPress);
		
		g_objWrapper.off("mousewheel", onMouseWheel);
