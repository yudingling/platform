...
//>>built
define("esri/geometry/Extent", "dojo/_base/declare dojo/_base/array dojo/_base/lang dojo/has ../kernel ../lang ../SpatialReference ./Geometry ./Point ./webMercatorUtils ./mathUtils".split(" "), function (x, v, p, y, w, z, s, A, m, t, B) {
    var u = {type: "extent", xmin: 0, ymin: 0, xmax: 0, ymax: 0}, g = x(A, {
        declaredClass: "esri.geometry.Extent", constructor: function (a, c, b, d, e) {
            p.mixin(this, u);
        ...
        getWidth: function () {
            ...
            var c = this.getCenter(), b = a.x - c.x;
            a = a.y - c.y;
            ...
            if (!a)return !1;
            var c = a.type, b = this.spatialReference, d = a.spatialReference;
            ...
            var c = a.points.length, b;
            for (b = 0; b < c; b++)if (this.contains(a.getPoint(b)))return !0;
            return !1
            ...
            var c = [this.xmin, this.ymax], b = [this.xmax, this.ymax], d = [this.xmin, this.ymin], e = [this.xmax, this.ymin], f = [c, b, d, e], c = [[d, c], [c, b], [b, e],
                [e, d]], d = a.rings, e = d.length, k, g = new m(0, 0, this.spatialReference);
            k = f.length;
            for (b = 0; b < k; b++)if (g.update(f[b][0], f[b][1]), a.contains(g))return !0;
            g.setSpatialReference(a.spatialReference);
            for (var l, h, b = 0; b < e; b++)if (f = d[b], k = f.length) {
                l = f[0];
                g.update(l[0], l[1]);
            ...
            var c = [[[this.xmin, this.ymin], [this.xmin, this.ymax]], [[this.xmin,
                this.ymax], [this.xmax, this.ymax]], [[this.xmax, this.ymax], [this.xmax, this.ymin]], [[this.xmax, this.ymin], [this.xmin, this.ymin]]], b, d = a.paths, e = d.length, f, k, g, l, h = new m(0, 0, a.spatialReference);
            for (a = 0; a < e; a++)if (f = d[a], k = f.length) {
                g = f[0];
                h.update(g[0], g[1]);
            ...
            var b = B._getLineIntersection2, d, e = c.length;
            for (d = 0; d < e; d++)if (b(a, c[d]))return !0;
            return !1
        }, contains: function (a) {
            if (!a)return !1;
            var c = a.type;
            if ("point" === c) {
                var b = this.spatialReference, d = a.spatialReference, c = a.x;
                a = a.y;
            ...
            var c = a.xmin, b = a.ymin, d = a.xmax, e = a.ymax, f = a.spatialReference;
            a = new m(c, b, f);
            c = new m(c, e, f);
            e = new m(d, e, f);
            b = new m(d,
            ...
            if (!this._shifted) {
                var c = new g(this.toJson()), b = c.spatialReference;
                if (a = a || b._getInfo()) {
                    var d = this._getCM(a);
                    if (d) {
                        var e = b._isWebMercator() ? t.webMercatorToGeographic(d) : d;
                        c.xmin -= d.x;
                        c.xmax -= d.x;
                        b._isWebMercator() || (e.x = this._normalizeX(e.x, a).x);
                        c.setSpatialReference(new s(z.substitute({Central_Meridian: e.x}, 4326 === b.wkid ? a.altTemplate : a.wkTemplate)))
                    }
                }
                this._shifted = c
            }
            ...
            var c, b = a.valid[0];
            a = a.valid[1];
            var d = this.xmin, e = this.xmax;
            if (!(d >= b && d <= a) || !(e >= b && e <= a))c = this.getCenter();
            return c
        }, _normalize: function (a, c, b) {
            var d = new g(this.toJson()), e = d.spatialReference;
            if (e &&
                (b = b || e._getInfo())) {
            ...
            if (!this._parts) {
                var c = this.xmin, b = this.xmax, d = this.ymin, e = this.ymax, f = this.spatialReference, k = this.getWidth(), m = c, l = b, h = 0,
                    n = 0, p = [], q, r;
                a = a || f._getInfo();
                q = a.valid[0];
                r = a.valid[1];
                ...
                if (k > 2 * r) {
                    ...
                    p.push({extent: k, frameIds: [h]}, {extent: c, frameIds: [n]}, {extent: r, frameIds: f}, {
                        extent: d,
                        frameIds: q
                    })
                } else c >
                b || a ? p.push({extent: new g(c, d, r, e, f), frameIds: [h]}, {
                    extent: new g(q, d, b, e, f),
                    frameIds: [n]
                }) : p.push({extent: new g(c, d, b, e, f), frameIds: [h]});
                this._parts = p
            }
            return this._parts
        }, _normalizeX: function (a, c) {
            var b = 0, d = c.valid[0], e = c.valid[1], f = 2 * e;
            a > e ? (b = Math.ceil(Math.abs(a - e) / f), a -= b * f) : a < d && (b = Math.ceil(Math.abs(a - d) / f), a += b * f, b = -b);
            return {x: a, frameId: b}
        }, toJson: function () {
            var a = {xmin: this.xmin, ymin: this.ymin, xmax: this.xmax, ymax: this.ymax}, c = this.spatialReference;
            c && (a.spatialReference = c.toJson());
            return a
        }
    });
    g.defaultProps = u;
    y("extend-esri") && (p.setObject("geometry.Extent", g, w), w.geometry.defaultExtent = u);
    return g
});