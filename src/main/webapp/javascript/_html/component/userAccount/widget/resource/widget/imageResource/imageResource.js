...
define([
    ...
    "dojo/text!./template/imageResource.html",
    "tool/css!./css/imageResource.css",
], function(
        base,
        declare,
        _Widget,
    ...
    return declare("component.userAccount.widget.resource.widget.imageResource", [_Widget], {
        baseClass: "component_userAccount_widget_resource_imageResource",
        ...
        _initDom: function(){
            $(this.domNode).find('.smsHead>a').click(lang.hitch(this, function(){
                topic.publish('component/userAccount/widget/resource/purchase', {resourceTp: 'image', minSize: 1000});
            }));
        },
        
        _setData: function(){
            base.ajax({
                url: base.getServerNM() + 'platformApi/own/user/normal/resourceStatus/image'
        ...
        _updateStatus: function(status){
            ...
            if(status.total < 0){
                $(this.domNode).find('.processDesc').html('当前存储 ' + status.current +' 张，无限制');
            }else{
                var remain = status.total - status.current;
                if(remain < 0){
                    remain = 0;
                }
                $(this.domNode).find('.processDesc').html(remain + ' 张剩余，共 ' + status.total + ' 张')
            }
        },
        ...
        _createChart: function(stm, etm, statistic){
            ...
            this.chart = echarts.init($(this.domNode).find('.imageChart')[0]);
