...
(function ($, window, document, undefined)
{
    ...
    var pluginName = "pwstabs",
            defaults = {
                tabChange: null,            //eidt by zd, tab 改变前的事件, function(preTab, currentTab), return false 可以禁止其切换
    ...
    Plugin.prototype = {
        ...
        init: function ()
        {
             ...
             *  edit by zd ,don't need size check
             */
//            if (this.settings.tabsPosition == 'vertical') {
...
//            }
            ...
            controller.on('click', function(e){
                that.liClick(e, that.container, that.settings.tabChange);
            });
        ...
        liClick: function (e, container, tabChange) {

            // Prevent use of href attribute;
            e.preventDefault();

            var preTabId = container.find('a.pws_tab_active').attr('data-tab-id');
            var tabId = $(e.currentTarget).attr('data-tab-id');

            if(tabChange && preTabId != tabId && !tabChange(preTabId, tabId)){
                return;
            }

            // Remove active class from all controllers
            container.find('a.pws_tab_active').removeClass('pws_tab_active');
            // Add active class to current controller
            $(e.currentTarget).addClass('pws_tab_active');

            var currentTab = container.find('.pws_tabs_list div[data-pws-tab="' + tabId + '"]');

            container.find('.pws_tabs_list div[data-pws-tab]').removeClass('pws_show').hide();
            currentTab.show().addClass('pws_show');
        },
        
        addTab: function(args){
            //class="pws_tab_active"
            var li = $('<li><a data-tab-id="'+ args.id +'" >' + args.name + '</a></li>');
            
            var tab = $('<div></div>');
            tab.attr('data-pws-tab', args.id).attr('data-pws-tab-name', args.name)
            tab.addClass(args.id).addClass('pws_hide').addClass('pws_tab_single').hide();
            
            this.container.find('ul.pws_tabs_controll').append(li);
            this.container.find('div.pws_tabs_list').append(tab);
        ...
        deleteTab: function(args){
            
            var nextActive = false;
            var curA = this.container.find('a[data-tab-id="' + args.id + '"]');
            if(curA.hasClass('pws_tab_active')){
                nextActive =  true;
            }
            
            curA.parent().remove();
            this.container.find('.pws_tabs_list div[data-pws-tab="' + args.id + '"]').remove();
            
            if(nextActive){
                this.container.find('a[data-tab-id]').first().click();
            }
        }
...
//    };
    ...
    $.fn[pluginName] = function (options) {
        ...
        return {
            //args: {id: 'xxxx', name: 'xxxxs', active: true/false, success: 'callback func after appended'}
            'addTab': function(args){
                $.each(plugins, function(index, plg){
                    plg.addTab(args);
                });
            },
            
            //args: {id: 'xxxx', name: 'xxxxs'}
            'deleteTab': function(args){
                $.each(plugins, function(index, plg){
                    plg.deleteTab(args);
                });
