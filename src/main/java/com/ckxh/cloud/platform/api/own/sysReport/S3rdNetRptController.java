...
@Scope("singleton")
@Controller
@RequestMapping("/own/sysReport/s3rdNetRpt")
public class S3rdNetRptController {
	@Autowired
	...
	public String get(@RequestParam String selTp, @RequestParam int tmLen, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			statusMap.put("netAmount", this.sysReportService.getThirdPartyServiceNetAmount());
			...
			Map<String, Double> statistic = this.sysReportService.getThirdPartyServiceNetStatistic(stm_ts, etm_ts, selTp);
