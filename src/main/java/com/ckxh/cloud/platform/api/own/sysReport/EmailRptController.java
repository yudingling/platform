...
@Scope("singleton")
@Controller
@RequestMapping("/own/sysReport/emailRpt")
public class EmailRptController {
	@Autowired
	...
	public String get(@RequestParam String selTp, @RequestParam int tmLen, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			statusMap.put("successCount", this.sysReportService.getEmailSendCount(PushStatus.success));
			statusMap.put("sendingCount", this.sysReportService.getEmailSendCount(PushStatus.sending));
			statusMap.put("failCount", this.sysReportService.getEmailSendCount(PushStatus.fail));
			...
			Map<String, Long> statistic = this.sysReportService.getEmailSendStatistic(stm_ts, etm_ts, selTp);
