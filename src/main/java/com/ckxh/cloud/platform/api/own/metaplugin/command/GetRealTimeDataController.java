...
@Scope("singleton")
@Controller
@RequestMapping("/own/command/getRealTimeData")
public class GetRealTimeDataController {
	@Autowired
	...
	public String post(@RequestParam String clientId, @RequestParam String metaId, @RequestParam String metaCId, 
		...
		try {
			...
			GetRealTimeDataCmd cmd = new GetRealTimeDataCmd(clientId, cli.getDEVICE_ID(), DateUtil.getCurrentTS(), metaCId);
