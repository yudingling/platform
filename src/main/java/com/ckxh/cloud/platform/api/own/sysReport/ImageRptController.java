...
@Scope("singleton")
@Controller
@RequestMapping("/own/sysReport/imageRpt")
public class ImageRptController {
	@Autowired
	...
	public String get(@RequestParam String selTp, @RequestParam int tmLen, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			statusMap.put("imageCount", this.sysReportService.getImageCount());
			double[] rechargeArr = this.sysReportService.getUserRechargeStatistic(RechargeType.image, PayResultType.success);
			...
			Map<String, Long> statistic = this.sysReportService.getImageStatistic(stm_ts, etm_ts, selTp);
