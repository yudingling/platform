...
@Scope("singleton")
@Controller
@RequestMapping("/open/maintUser/verify")
public class MaintUserVerifyController {
	@Autowired
	...
	public String post(@RequestParam String qa, @RequestParam String pwd, @RequestParam String phone, 
			@RequestParam String verifyCode, @RequestParam(required = false) Long maId, 
		...
		try{
			...
			if(uid == null || uid.length() == 0){
				throw new Exception("验证码超时, 请联系所属用户重新发送验证邮件");
			}
			
			if(!this.maintenanceService.isAreaCanSetToMaintUser(uid, maId)){
				throw new Exception("参数错误");
			}
			...
			if(this.userInfoService.msgVerify(phone, verifyCode)){
				...
				if(user != null && user.getU_ACTIVE() == 0 && user.getU_DISABLED() == 0 
					...
					this.userEffectiveTool.afterMaintUserVerify(user, maId);
