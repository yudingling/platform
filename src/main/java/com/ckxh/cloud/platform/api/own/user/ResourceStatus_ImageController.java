...
@Scope("singleton")
@Controller
@RequestMapping("/own/user/normal/resourceStatus/image")
public class ResourceStatus_ImageController {
	@Autowired
	...
	public String get(HttpServletRequest request, HttpServletResponse response){
		try{
			...
			retMap.put("status", this.resourceStatusGetter.getStatus(RechargeType.image, uid));
			
			//image file statistic in 30 days
			...
			Map<String, Long> statistic = this.streamDataService.getDailyStatisticOfImage(uid, stm_ts, etm_ts);
