...
@Scope("singleton")
@Controller
@RequestMapping("/own/sysReport/userRpt")
public class UserRptController {
	@Autowired
	...
	public String get(@RequestParam String selTp, @RequestParam int tmLen, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			statusMap.put("userCount", this.sysReportService.getUserCount());
			statusMap.put("clientCount", this.sysReportService.getClientCount());
			...
			Map<String, Long> statistic = this.sysReportService.getUserStatistic(stm_ts, etm_ts, selTp);
