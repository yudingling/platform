...
@Scope("singleton")
@Controller
@RequestMapping("/own/sysReport/s3rdPurchaseRpt")
public class S3rdPurchaseRptController {
	@Autowired
	...
	public String get(@RequestParam String selTp, @RequestParam int tmLen, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			statusMap.put("purchaseAmount", this.sysReportService.getThirdPartyServicePurchaseAmount(PayResultType.success));
			...
			Map<String, Double> statistic = this.sysReportService.getThirdPartyServicePurchaseStatistic(stm_ts, etm_ts, selTp);
